...
namespace Infrastructure.ObjectModel
{
    ...
    /// in the hierarchy.
    /// </summary>
    public abstract class CompositeDrawableComponent<T> :
        DrawableGameComponent, ICollection<T>
        where T : IGameComponent
    {
        // the entire collection, for general collection methods (count, foreach, etc.):
        Collection<T> m_Components = new Collection<T>();

        #region Selective Collections
        // selective holders for specific operations each frame:
        private List<T> m_UninitializedComponents = new List<T>();
        ...
        #region Events
        public event EventHandler<GameComponentEventArgs<T>> ComponentAdded;
        public event EventHandler<GameComponentEventArgs<T>> ComponentRemoved;
        #endregion //Events

        #region Add/Remove
        protected virtual void OnComponentAdded(GameComponentEventArgs<T> e)
        {
            if (m_IsInitialized)
            {
        ...
        protected virtual void OnComponentRemoved(GameComponentEventArgs<T> e)
        {
            if (!m_IsInitialized)
            {
        ...
        private void InitializeComponent(T i_Component)
        {
        ...
        {
            int count = m_UpdateableComponents.Count;

            for (int i = 0; i < m_UpdateableComponents.Count; i++)
            {
                IUpdateable updatable = m_UpdateableComponents[i];
                if (updatable.Enabled && count == m_UpdateableComponents.Count)
                {
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            m_SpriteBatch.Begin(
                this.SpritesSortMode, BlendState.NonPremultiplied, this.SamplerState,
        ...
        public virtual void Add(T i_Component)
        {
            this.InsertItem(m_Components.Count, i_Component);
        }

        protected virtual void InsertItem(int i_Idx, T i_Component)
        {
            ...
            if (i_Component != null)
            {
                ...
                OnComponentAdded(new GameComponentEventArgs<T>(i_Component));
            }
        }
        ...
        public void Clear()
        {
            for (int i = 0; i < Count; i++)
            {
                OnComponentRemoved(new GameComponentEventArgs<T>(m_Components[i]));
            }

            m_Components.Clear();
        }

        public bool Contains(T i_Component)
        {
            return m_Components.Contains(i_Component);
        }

        public void CopyTo(T[] io_ComponentsArray, int i_ArrayIndex)
        {
        ...
        public virtual bool Remove(T i_Component)
        {
            bool removed = m_Components.Remove(i_Component);

            if (i_Component != null && removed)
            {
                OnComponentRemoved(new GameComponentEventArgs<T>(i_Component));
            }
